...
from entities.base_image import BaseImage
...
class Image(BaseImage):
  def __init__(self, path, cluster = None):
    ...
    self._color_2_num_repr(cluster)
  ...
  def _color_2_num_repr(self, cluster = None):
    numeric_repr = self._set_cluster(cluster) # flat.
    super().__init__(numeric_repr.reshape(self.cv_image.shape[:-1])) # 2d.
  ...
  def get_sub_image(self, offset, dimensions):
    x, y = offset
    width, height = dimensions
    return BaseImage(self.source[y:y+height, x:x+width])
